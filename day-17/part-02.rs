...
// is the presence of the 4th dimension field.
// This code isn't the fastest but with -O it got the
...
impl Point {
    ...
    // The size of the result vector is always 80.
            ...
            for dy in -1..2 {
                for dz in -1..2 {
                    for dw in -1..2 {
                        ...
                        adj.push(Point(self.0 + dx, self.1 + dy, self.2 + dz, self.3 + dw));
                    }
                }
            }
        }
...
fn main() -> io::Result<()> {
    ...
    for line in read_file("input.txt")? {
        ...
        for ch in line {
            if ch == '#' {
                state.add_active_point(Point(i, j, 0, 0));
            }
            j += 1;
        }
        i += 1;
